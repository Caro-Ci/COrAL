...
class MIMIC(Dataset):
    ...
    def __init__(self, data_path: str = "im.pk",
        ...
        #print(f"length: {len(next(iter(datafile.values())))}")
        ...
        if flatten_time_series:
            ...
            #Change by me, adding binary mortality, ignoring icd classification
        if task == -1:
            y= datafile['adm_labels_mort']
            #before elif was just if
        elif task < 0:
