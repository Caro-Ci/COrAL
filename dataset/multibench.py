...
from dataset.multimodal_manipulation import MultimodalManipulationDataset
import math
...
class MultiBenchDataModule(LightningDataModule):
    ...
    def __init__(self, dataset: str,
        ...
        """
        Args:
            dataset: in {"mimic", "mosi", "mosei", "humor", "sarcasm"}
            model: in {'Sup', 'SupervisedClassifier','COrAL',}
                The model defines the augmentations to apply:
                    - Sup, SupervisedClassifier: no augmentation, returns the modalities + label
                    - COrAL: augmentation for each modality, returns pairs of augmented modalities
        ...
        if self.model == "Sup":
            self.train_dataset = MultiBench(self.dataset, "train", "downstream", **self.dataset_kwargs)
            self.val_dataset = MultiBench(self.dataset, "val", "downstream",**self.dataset_kwargs)
            self.test_dataset = MultiBench(self.dataset, "test","downstream", **self.dataset_kwargs)
        elif self.model == "COrAL":
            ...
            self.train_dataset = MultiBenchSupCon(self.dataset, "train", **self.dataset_kwargs)
            self.val_dataset = MultiBenchSupCon(self.dataset, "val", **self.dataset_kwargs)
            self.test_dataset = MultiBenchSupCon(self.dataset, "test", **self.dataset_kwargs)
        else:
...
class MultiBench(Dataset):
    def __init__(self, dataset: str, split: str, mode: str = "training", **dataset_kwargs):
        super().__init__()
